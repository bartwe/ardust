...
public class Character {
    ...
    public void animateMining()
    {
        switch (movementDirection)
        {
            case UP: sprite.animate(36,4,Constants.DWARF_ANIMATION_SPEED / 2); break;
            default: sprite.animate(32,4,Constants.DWARF_ANIMATION_SPEED / 2);break;
        }
    }
    ...
    public void draw(Painter p, Point viewportLocation, boolean selectedDwarf)
    {
        ...
        if (isMoving())
        {
            switch (movementDirection)
            {
                case UP: localPoint.y += (Constants.TILE_BASE_HEIGHT - (int)betweenTilePosition); break;
                case RIGHT: localPoint.x -= (Constants.TILE_BASE_WIDTH - (int)betweenTilePosition); break;
                case LEFT: localPoint.x += (Constants.TILE_BASE_WIDTH - (int)betweenTilePosition); break;
                case DOWN: localPoint.y -= (Constants.TILE_BASE_HEIGHT - (int)betweenTilePosition); break;
            }
        }
        if (selectedDwarf) p.draw(localPoint.x, localPoint.y  - (Constants.TILE_DRAW_HEIGHT - Constants.TILE_BASE_HEIGHT), 96, 40, 43, 7, false);//sorry
